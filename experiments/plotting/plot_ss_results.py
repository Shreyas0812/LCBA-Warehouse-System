...
plot_ss_results.py
==================
Compact views for steady-state experiment results.

This script summarizes a steady-state run with a small number of figures:
1. Heatmaps per metric, one subplot per algorithm.
   - Primary axes: task_arrival_rate (X) × comm_range (Y)
2. Stability curve: max arrival rate achieving a throughput threshold,
   plotted against comm_range.
3. Tradeoff scatter: throughput vs avg task wait time.
4. Throughput degradation curves, one subplot per comm_range.
5. Wait-time degradation curves, one subplot per comm_range.

Outputs are written directly to:
    results/experiments/<map_name>/<timestamp>/ss_results/compact_views/

Usage:
    python experiments/plotting/plot_ss_results.py --csv path/to/summary.csv --all --save
    python experiments/plotting/plot_ss_results.py --csv path/to/summary.csv --heatmaps --save
    python experiments/plotting/plot_ss_results.py --csv path/to/summary.csv --capacity --save
    python experiments/plotting/plot_ss_results.py --csv path/to/summary.csv --tradeoff --save
    python experiments/plotting/plot_ss_results.py --csv path/to/summary.csv --degradation --save
"""
...
HEATMAP_METRICS = [
    ...
    ("idle_ratio", "Avg Idle Ratio"),
    ("allocation_time_ms", "Avg Allocation Time (ms)"),
]
...
def write_run_manifest(output_dir: str, command_text: str, metadata: dict) -> None:
    ...
    with open(manifest_path, "w", encoding="utf-8") as f:
        f.write("Steady-State Plot Run Manifest\n")
        f.write("==============================\n")
    ...
    for idx in range(4):
        ...
        for r in range(piv.shape[0]):
            for c in range(piv.shape[1]):
                ...
                if metric in ("completion_rate", "idle_ratio"):
                    label = f"{float(v):.2f}"
                elif metric == "allocation_time_ms":
                    label = f"{float(v):.2f}"
                elif metric in ("throughput", "wait_time", "queue_depth"):
