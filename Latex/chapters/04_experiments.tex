...
\begin{itemize}
  ...
  \item \textbf{Steady-state arrival fractions} in full mode:
    $\{0.5, 0.7, 0.85, 1.0, 1.1\}$ times the map-specific
...
\subsubsection*{Steady-State Figures (Pending)}

Steady-state results are being generated in a separate run. The final chapter version
will add steady-state figures after those results are available, following the same
reliability-first policy used for batch analysis.
...
are available in the detailed plot suites and are used where relevant in the
