...
of its four cardinal neighbors or remain stationary (Hold Position).
...
the objective is to minimize makespan: the timestep at which the last task is
completed.
In \emph{steady-state mode}, tasks arrive continuously at each induct station at
rate $\lambda$ tasks per timestep, and the objective is to maximize throughput:
the number of tasks completed per timestep over the full 1500-timestep window
(the reported runs use $\text{warmup\_timesteps} = 0$).
...
within its communication neighborhood $\mathcal{N}_i(t) = \{k : (i,k) \in E(t)\}$.
...
and a \emph{path planner}, integrated through a centralized
...
\begin{figure}[htbp]
  ...
  \caption{System architecture overview. The \texttt{IntegrationOrchestrator}
           ...
           path planners. Allocation is logically decentralized (agents run
...
It is \emph{logically decentralized}: each agent maintains its own bid and
...
rules for each agent using only the neighborhood defined by $G(t)$.

The path planner (Section~\ref{sec:method_collision}) is \emph{physically
centralized} in the simulation: the orchestrator has global visibility of
all agent positions and plans collision-free paths for all agents using
a shared space-time reservation table.
% This design choice --- decentralized allocation, centralized collision
...
% decisions, which is exactly the behavior we wish to study.
...
\subsection{Per-Timestep Execution Loop}
\label{sec:method_execution_loop}

Figure~\ref{fig:system_flowchart} illustrates the complete per-timestep
sequence that orchestrates the system components shown in Figure~\ref{fig:system_architecture}.
At each timestep, the orchestrator injects newly arriving tasks, triggers the
allocation algorithm conditionally based on system events, classifies agent roles
(task execution, charging, or idle), selects the appropriate path planner for
each role, executes one step of agent motion with collision checking, and
manages energy and charging transitions.
This architecture tightly couples allocation decisions with execution state,
enabling dynamic replanning at critical points: when agents complete tasks,
charge transitions occur, or collisions require local repair.

\begin{figure}[htbp]
  \centering
  \resizebox{0.90\textwidth}{!}{%
    \input{figures/IntegrationOrchestrator_flowchart}%
  }
  \caption{Per-timestep execution loop of the \texttt{IntegrationOrchestrator}.
           The flowchart shows the sequence of decision points and execution
           phases during each simulation timestep. Allocation is triggered
           conditionally (Task Injection, energy events, etc.); once triggered,
           bundle construction and consensus run to completion, followed by
           role-based path planning and agent execution.}
...
summarized in Table~\ref{tab:environments}.
...
All agents are initialized at their home positions at $t = 0$.
...
environment; results across all six environments are reported in the
  scalability experiments (Section~\ref{sec:exp_setup}).
...
The energy configuration is validated at initialization: if any reachable
...
This behavior mirrors industrial deployments such as Amazon Robotics and
...
The BFS tables are computed once at initialization and shared across all agents.
...
decentralized convergence detection mechanism.
...
\begin{equation}
  S_i^{\text{RPT}}(p_i) = -\sum_{j \in p_i}
    \tau_{ij}\!\bigl(p_i^{\le j}\bigr),
  \label{eq:rpt}
\end{equation}
where $p_i$ is an ordered sequence of tasks assigned to agent $i$,
$p_i^{\le j}$ denotes the subsequence of $p_i$ up to and including task $j$,
and $\tau_{ij}(p_i^{\le j})$ is the \emph{completion time} of task $j$
...
Intuitively, RPT penalizes both the number of tasks and their cumulative
...
CBBA performs a single round of neighbor-to-neighbor information exchange.
...
%% ── 3.4.4 ──────────────────────────────────────────────────────
\subsection{Decentralized Convergence Detection}
...
GCBBA includes a decentralized convergence detection mechanism to allow early
...
initialization and stored as a distance table.
...
same cell (wait action) or a cardinal neighbor of $(x, y)$ in the
obstacle-free grid.
...
Phase~2.
Within each phase, the planning order is randomized to avoid systematic priority
...
RHCR is parameterized by a \emph{window size} $w$ (the number of timesteps
...
Table~\ref{tab:planner_comparison} summarizes the three planners along four
...
planners without modification.
  For the allocation comparison experiments (Section~\ref{chap:experiments}),
  CA* is held fixed across all methods as an
...
used to compare planner behavior across maps.
...
Figure~\ref{fig:io_flowchart} summarizes this loop visually.

\begin{figure}[htbp]
  \centering
  \resizebox{0.92\linewidth}{!}{\input{figures/IntegrationOrchestrator_flowchart.tex}}

  \caption{IntegrationOrchestrator per-timestep simulation loop. At each
           timestep ($t = 0, 1, 2, \ldots$), the system injects tasks in
           steady-state mode, checks allocation triggers, optionally invokes
           LCBA, classifies agent roles, selects a planner per role, executes
           collision-free paths, manages energy, and records metrics. Color
           coding is used for start/end, process, and decision nodes.}
  \label{fig:io_flowchart}
\end{figure}
...
\begin{enumerate}
  \item \textbf{Initialization.} At $t = 0$ (or after a system reset),
  ...
  \item \textbf{New tasks and idle agents.} If new tasks have been injected
        ...
        allocation is triggered immediately to minimize task wait time.
...
SGA is the centralized optimality reference.
...
which is a generous assumption --- a centralized algorithm would, in practice,
...
DMCHBA~\cite{samiei2024dmchba} is a state-of-the-art decentralized task
...
settings that characterize the LCBA evaluation.
...
\textbf{Hybrid decentralized allocation / centralized path planning.}
As argued in Section~\ref{sec:method_overview}, task allocation is kept
decentralized to faithfully model communication constraints, while path planning
is centralized to avoid the added complexity of distributed reservation table
synchronization.
This is a deliberate choice to isolate the allocation variable in experiments.
A fully decentralized path planner (such as distributed PIBT) would be an
