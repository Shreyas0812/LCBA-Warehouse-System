...
\begin{tikzpicture}[node distance=20mm, >=Stealth, every node/.style={font=\small}]
  ...
  \node[module, above=10mm of io] (gcbba) {LCBA \\ (Task Allocator)};
  ...
  \node[draw, rounded corners, below=10mm of io, minimum width=70mm, minimum height=14mm, align=center, fill=yellow!6] (agents) {Agents / Local Comm. \\ (per-agent bid vectors, local neighborhood G(t))};
  ...
  % agents -> gcbba (dashed, neighborhood msgs): route right of planner, then up, then left
  \draw[arrow, dashed] (agents.east) -- ++(55mm,0) |- (gcbba.east)
      node[pos=0.82, above, font=\scriptsize] {neighborhood msgs (simulated)};
