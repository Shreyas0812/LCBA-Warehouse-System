...
\begin{frame}{LCBA: Three Key Relaxations over GCBBA}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \subheader{\textcolor{TextMuted}{01}\hspace{6pt}Component-Local Consensus}\par\vspace{2pt}
      ...
      \subheader{\textcolor{TextMuted}{02}\hspace{6pt}Timestamped Conflict Resolution}\par\vspace{2pt}
      ...
      \subheader{\textcolor{TextMuted}{03}\hspace{6pt}Event-Driven Replanning}\par\vspace{2pt}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{infobox}[top=4pt, bottom=4pt]
        \textbf{Theoretical guarantee.} On connected components, LCBA inherits GCBBA's convergence and greedy-style \textbf{allocation-utility} guarantee (worst-case $\leq 2\times$ from optimal under standard assumptions).
      \end{infobox}
      
      % Research questions moved to their own slide to improve readability.
    \end{column}
  \end{columns}
\end{frame}

%% ── Slide 7: Research Questions (separate slide for clarity) ──
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.96\textwidth}
      \begin{plaincard}{}
        {\rmfamily\small
        \begin{tightitemize}
          \item \rqlabel{RQ1}\, Can LCBA maintain task throughput under communication conditions where static allocation fails?
          \item \rqlabel{RQ2}\, How close does LCBA stay to the SOTA reference (DMCHBA) as communication connectivity varies?
          \item \rqlabel{RQ3}\, Does LCBA's robustness generalize across diverse warehouse environments and scales?
        \end{tightitemize}}
      \end{plaincard}
    \end{column}
  \end{columns}
...
%% ── Slide 8a: Bundle Construction ───────────────────────────
\begin{frame}{LCBA Algorithm: Bundle Construction}
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      \begin{plaincard}{Practical note}
        \smallcap{Bundle construction is lightweight and bounded by the bundle length $L_{\max}$.}
      \end{plaincard}
      \vspace{8pt}
      \begin{infobox}
        \textbf{Tip.} Early termination in insertion reduces compute by
        avoiding full-scan marginal gains when no beneficial insertions remain.
      \end{infobox}
    \end{column}
  \end{columns}
\end{frame}

%% ── Slide 8b: Local Consensus ──────────────────────────────
\begin{frame}{LCBA Algorithm: Local Consensus}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}\raggedright
...
%% ── Slide 9: Reconnection & Event-Driven Replanning ──────────
\begin{frame}{LCBA Algorithm: Reconnection \& Event-Driven Replanning}
  \begin{columns}[T]
    ...
    \vspace{-15pt}
      \begin{plaincard}[top=4pt, bottom=4pt]{LCBA Event-Driven Replanning Triggers:}
        {\rmfamily\footnotesize
        \begin{tightenum}
          \item $t = 0$ or restart.
          \item Charging state changes.
          \item New tasks arrive \textsc{and} idle agents exist.
          \item $\geq \lceil \Na/3 \rceil$ completions since last run
                \textsc{and} cooldown done.
          \item Pending tasks remain \textsc{and} idle agents exist
                \textsc{and} cooldown done.
        \end{tightenum}}
      \end{plaincard}
      \vspace{-5pt}
      \begin{infobox}[top=4pt, bottom=4pt]
        ...
        Triggers 3--5 adapt naturally to task rate.
      \end{infobox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{System Architecture: Full Coordination Pipeline}
  ...
  \resizebox{0.99\linewidth}{!}{%
    \input{figures/IntegrationOrchestrator_flowchart_slide_3part.tex}%
  }
\end{frame}

\begin{frame}{Operational Safeguards}
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
    ...
    \begin{column}{0.32\textwidth}
      \begin{plaincard}{Energy-Budget Filter}
        \smallcap{LCBA skips bids on tasks an agent cannot complete
    ...
    \begin{column}{0.45\textwidth}\raggedright
      ...
      \begin{plaincard}{Steady-State}
        \smallcap{Continuous arrivals; throughput over 1500 ts;
                  Metrics: throughput,
                  mean latency, 95\%-latency, completion rate,
                  agent utilization.}
      \end{plaincard}
    \end{column}
  \end{columns}
...
\begin{frame}{Baselines}
  \begin{columns}[T]
    \begin{column}{0.33\textwidth}
      \begin{plaincard}{CBBA}
        \smallcap{Decentralized auctions with pairwise consensus. Local
                  decisions; robust to partial connectivity but may
                  fragment into components.}
      \end{plaincard}
    \end{column}
    \begin{column}{0.33\textwidth}
      \begin{plaincard}{SGA}
        \smallcap{Centralized greedy reference. High performance when
                  fully connected; degrades to per-component greedy
                  behavior under disconnection.}
      \end{plaincard}
    \end{column}
    \begin{column}{0.33\textwidth}
      \begin{plaincard}{DMCHBA}
        \smallcap{State-of-the-art decentralized matching; uses local
                  Hungarian matching per component for near-optimal
                  assignments locally.}
      \end{plaincard}
    \end{column}
  \end{columns}
...
%% ── Slide 12: Six Evaluation Environments ────────────────────
...
%% ── Slide 13: Batch Completion Rate Heatmaps ─────────────────
\begin{frame}{Batch Results: Completion Rate Heatmaps}
  ...
  \begin{columns}[T]
    ...
    \vspace{-0.2cm}
      \begin{infobox}
        \textbf{LCBA. (top-left)} 100\% completion at \emph{all} comm ranges
        at light load; reliable from $\comm=8$ upward at moderate load.
      \end{infobox}
      \vspace{2pt}
      \begin{warnbox}
        \textbf{CBBA. (top-right)} Fails on both seeds at $\comm=4$ and
        $\comm=25$ at 96 tasks; timeouts grow sharply with load.
      \end{warnbox}
      \vspace{2pt}
      \begin{dangerbox}
        \textbf{DMCHBA. (bottom-left)} $\approx$2725\,ts at $\comm=4$ vs.\ 515\,ts
        connected.
      \end{dangerbox}
    \end{column}
  \end{columns}
...
%% ── Slide 14: Batch Completion Degradation by Comm Range ─────
\begin{frame}{Batch Results: Completion Degradation by Comm Range}
  \vspace{-0.5cm}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{mapcard}{Warehouse Small}
        \includegraphics[width=\linewidth, height=5.9cm, keepaspectratio]%
  ...
  \vspace{10pt}
  \begin{infobox}
    Each line is a communication range. Flat
...
\begin{frame}{Batch Results: Makespan and Sub-Optimality (RQ2)}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      ...
      \begin{infobox}
        {\rmfamily\scriptsize
        \rqlabel{RQ2 answer.} On connected graphs,
        LCBA is within the same batch-time range as the SOTA reference (DMCHBA) on the connected case, while degrading much more gracefully as connectivity drops.
        % In the table, LCBA goes from 640 to 685\,ts (7\%), compared with DMCHBA's 515 to 2725\,ts (430\%).
        }
      \end{infobox}
    \end{column}

    \begin{column}{0.44\textwidth}\raggedright
      \subheader{Key observations}\par\vspace{2pt}
      {\rmfamily\footnotesize
      \begin{tightitemize}
        ...
        \item DMCHBA suffers most: \textbf{430\%} at fully
              disconnected because matching quality degrades as
              the graph fragments.
        \item SGA timed out on seed~42; per-component SGA is an
              imperfect reference when disconnected.
      \end{tightitemize}}
      \vspace{1pt}
      \begin{warnbox}
        {\rmfamily\scriptsize
        \textbf{Takeaway.} LCBA achieves near SOTA quality on
        connected graphs and degrades much more gracefully than
        baselines as connectivity drops.}
      \end{warnbox}
    \end{column}
  \end{columns}
\end{frame}
...
%% ── Slide 16: Steady-State Throughput Heatmap ────────────────
\begin{frame}{Steady-State: Throughput Heatmap (Warehouse Small)}
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}\raggedright
      \subheader{Heatmap}\par\vspace{2pt}
      {\rmfamily\footnotesize
      \begin{tightitemize}
      ...
      \begin{tightitemize}
        ...
        \item \textbf{DMCHBA:} drops sharply to 0.017\,tasks/ts at
              $\comm=4$ (vs.\ 0.067 connected)
        \item \textbf{CBBA:} moderate drop at
...
%% ── Slide 17: Steady-State Throughput Degradation ────────────
...
%% ── Slide 18: Capacity Frontier & Tradeoff ───────────────────
...
%% ── Slide 19: Allocation Efficiency ──────────────────────────
\begin{frame}{Allocation Efficiency: Speed Across the Spectrum}
  \begin{columns}[T]
    ...
    \end{column}
      ...
      \end{tightitemize}}
      \vspace{6pt}
      {\rmfamily\footnotesize\mutedtext{LCBA and CBBA converge in allocation cost at full
...
%% ── Slide 21: Conclusions / Contributions ────────────────────
\begin{frame}{Conclusions: Contributions}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \subheader{Five Contributions}\par\vspace{6pt}

      \subheader{\textcolor{TextMuted}{01}\hspace{6pt}LCBA Algorithm}\par\vspace{2pt}
      \smallcap{Component-local consensus + timestamped reconnection + event-driven replanning; inherits the $\leq 2\times$ allocation-utility worst-case guarantee.}

      \vspace{8pt}
      \subheader{\textcolor{TextMuted}{02}\hspace{6pt}Hybrid Pipeline}\par\vspace{2pt}
      \smallcap{Modular allocation + 3 interchangeable path planners; layers independently configurable.}

      \vspace{8pt}
      \subheader{\textcolor{TextMuted}{03}\hspace{6pt}Dual-Mode Simulation}\par\vspace{2pt}
      \smallcap{Batch (completion, makespan) and steady-state (throughput, wait, queue depth) in one environment.}

      \vspace{10pt}
      \subheader{\textcolor{TextMuted}{04}\hspace{6pt}Empirical Evaluation}\par\vspace{2pt}
      \smallcap{432+ runs, 6 environments, 6--200 agents, full factorial sweep.}

      \vspace{8pt}
      \subheader{\textcolor{TextMuted}{05}\hspace{6pt}Operational Realism}\par\vspace{2pt}
      \smallcap{Proactive charging, energy-budget bid filter, wait stations for idle agents.}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{infobox}
        ...
        \vspace{4pt}
        \rqlabel{RQ2.} On the connected case, LCBA is close to the
        SOTA baseline (DMCHBA), and it is much more stable under
        disconnection.

        \vspace{4pt}
        \rqlabel{RQ3.} Results generalize across 6 diverse
        environments (6--200 agents, 30$\times$30 to 161$\times$63);
        consistent robustness at all scales.
        }
      \end{infobox}
      \vspace{1pt}
      \begin{warnbox}
        {\rmfamily\footnotesize
        \textbf{Broader applicability.} Any domain where
...
%% ── Backup 4: Path Planner Comparison ─────────────────────────
\begin{frame}{Backup: Path Planner Comparison}
  \begin{columns}[T]
...
%% ── Backup 5: Steady-State Metrics Definitions ───────────────
